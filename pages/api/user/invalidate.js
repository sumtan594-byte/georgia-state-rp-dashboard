...
export default async function handler(req, res) {
  ...
  if (!userId && !invalidateGuildRoles) {
    return res.status(400).json({ error: 'userId or invalidateGuildRoles is required' });
  }

  invalidateUserRefreshCache(userId || null, { guildRoles: invalidateGuildRoles });

  return res.status(200).json({ success: true, userId: userId || null, invalidateGuildRoles });
}
