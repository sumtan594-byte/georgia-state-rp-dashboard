...
export function normalizeMarkingResult(raw) {
  ...
  const rawGoods = (Array.isArray(source.goods || source.strengths) ? (source.goods || source.strengths) : []).slice(0, 6).map(item => cleanText(item)).filter(Boolean);
  const rawBads = (Array.isArray(source.bads || source.weaknesses || source.improvements) ? (source.bads || source.weaknesses || source.improvements) : []).slice(0, 6).map(item => cleanText(item)).filter(Boolean);
