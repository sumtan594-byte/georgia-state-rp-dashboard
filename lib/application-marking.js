...
export function normalizeMarkingResult(raw) {
  ...
  if (Array.isArray(scoreSource)) {
    scoreItems = scoreSource;
  } else if (scoreSource && typeof scoreSource === 'object') {
    scoreItems = Object.entries(scoreSource).map(([id, value]) => (
      typeof value === 'number'
        ? { id, score: value, note: '' }
        : { id, ...(value || {}) }
    ));
  ...
  const byId = new Map(scoreItems.map(item => [item?.id || item?.questionId || item?.question_id, item]));
  ...
  const criticalFail = Boolean(source.criticalFail ?? source.critical_fail) || safetyScore === 0;
  const decision = score >= 26 && !criticalFail ? 'accepted' : 'denied';
  const spag = source.spag || source.spaG || source.writing || {};
  const spagScore = Math.max(0, Math.min(100, Math.round(Number(spag?.score) || 0)));
  ...
  return {
    ...
    requiresHumanReview: criticalFail || (source.aiAssessment || source.ai_assessment)?.risk === 'high' || (score >= 20 && score <= 28),
    questionScores,
    ...
    spag: { score: spagScore, label: cleanText(spag?.label, 'Not assessed'), summary: cleanText(spag?.summary) },
    aiAssessment: {
      risk: ['low', 'medium', 'high'].includes((source.aiAssessment || source.ai_assessment)?.risk) ? (source.aiAssessment || source.ai_assessment).risk : 'low',
      signals: (Array.isArray((source.aiAssessment || source.ai_assessment)?.signals) ? (source.aiAssessment || source.ai_assessment).signals : []).slice(0, 8).map(item => cleanText(item)).filter(Boolean),
      explanation: cleanText((source.aiAssessment || source.ai_assessment)?.explanation, 'No strong AI-writing indicators identified.'),
    },
