...
export const MARKING_QUESTIONS = [
  ...
  { id: 'scen_6', label: 'Prohibited suicidal roleplay', criteria: 'Treats the rooftop-jump threat as an in-game suicidal/self-harm roleplay scenario, stops the scene, explains that this roleplay is prohibited, gathers context/evidence, and applies or escalates the appropriate server punishment. It is wrong to allow the roleplay to continue. Do not interpret it as literal real-world self-harm unless the applicant explicitly says the player disclosed real-world intent.' },
...
- Judge spelling, punctuation, grammar, capitalisation, clarity and tone, but be proportionate. A few mistakes must not destroy an otherwise sound answer. Reduce by one point only when writing quality is noticeably below staff expectations; reserve severe reductions for writing that is difficult to understand or highly unprofessional. Do not reward length or polished filler.
- Score the content actually written. Do not infer missing actions.
- For scen_6, the prompt describes an in-game rooftop-jump roleplay. Suicidal/self-harm roleplay is prohibited. Reward applicants who stop it and follow server procedure. Penalise applicants who allow it to continue. Never criticise an applicant merely for treating this scenario as roleplay, and never use the phrase “Treats self-harm as roleplay” as a weakness.
- Set criticalFail=true for scen_6 only if the applicant explicitly permits, encourages, mocks, or deliberately ignores the prohibited suicidal roleplay.
- Recommend acceptance at 22/36 or above with no critical failure. AI-writing risk is not proof and must never by itself determine acceptance.
...
export function normalizeMarkingResult(raw) {
  ...
  const source = candidates.find(item => item && typeof item === 'object' && (
    item.questionScores || item.question_scores || item.scores
  ));
  if (!source) throw new Error('Model returned an incomplete marking result');

  const scoreSource = source.questionScores || source.question_scores || source.scores;
  let scoreItems;
  ...
  const criticalFail = Boolean(source.criticalFail ?? source.critical_fail);
  const decision = score >= 22 && !criticalFail ? 'accepted' : 'denied';
  ...
  const spagScore = Math.max(0, Math.min(100, Math.round(Number.isFinite(explicitSpagScore) ? explicitSpagScore : (score / 36) * 100)));
  const spagLabel = spagScore >= 85 ? 'Excellent' : spagScore >= 70 ? 'Good' : spagScore >= 55 ? 'Fair' : 'Needs improvement';
  ...
  return {
    ...
    questionScores,
    goods: (Array.isArray(source.goods || source.strengths) ? (source.goods || source.strengths) : []).slice(0, 6).map(item => cleanText(item)).filter(Boolean),
    bads: (Array.isArray(source.bads || source.weaknesses || source.improvements) ? (source.bads || source.weaknesses || source.improvements) : []).slice(0, 6).map(item => cleanText(item)).filter(Boolean),
    spag: {
      score: spagScore,
      label: cleanText(spag?.label, spagLabel),
      summary: cleanText(spag?.summary, 'SPaG and professionalism were integrated into every question score.'),
    },
    aiAssessment: {
      risk: (() => {
    ...
    feedback: cleanText(source.feedback || source.finalFeedback || source.final_feedback), model: AUTO_MARK_MODEL,
  };
}
