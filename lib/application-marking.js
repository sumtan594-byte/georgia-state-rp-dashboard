...
export function normalizeMarkingResult(raw) {
  ...
  const suppliedGoods = (Array.isArray(source.goods || source.strengths) ? (source.goods || source.strengths) : []).slice(0, 6).map(item => cleanText(item)).filter(Boolean);
  const suppliedBads = (Array.isArray(source.bads || source.weaknesses || source.improvements) ? (source.bads || source.weaknesses || source.improvements) : []).slice(0, 6).map(item => cleanText(item)).filter(Boolean);
