  ...
  properties: {
    questionScores: {
      ...
      items: {
        ...
        required: ['id', 'score', 'good', 'bad'],
        properties: {
          id: { type: 'string' }, score: { type: 'integer', minimum: 0, maximum: 3 },
          good: { type: 'string' }, bad: { type: 'string' },
        },
      },
    },
    goods: { type: 'array', minItems: 1, maxItems: 6, items: { type: 'string' } },
    bads: { type: 'array', minItems: 1, maxItems: 6, items: { type: 'string' } },
    spag: {
      type: 'object', additionalProperties: false, required: ['score', 'label', 'summary'],
      properties: { score: { type: 'integer', minimum: 0, maximum: 100 }, label: { type: 'string' }, summary: { type: 'string' } },
    },
    aiAssessment: {
    ...
    feedback: { type: 'string' }, criticalFail: { type: 'boolean' }, decisionReason: { type: 'string' },
  },
};
...
export function normalizeMarkingResult(raw) {
  ...
  const questionScores = MARKING_QUESTIONS.map(question => {
    ...
    return {
      ...
      good: cleanText(item.good, 'No specific strength identified.'),
      bad: cleanText(item.bad, 'No specific weakness identified.'),
    };
  });
