...
function getEnv(primary, fallback = null) {
  return process.env[primary] || (fallback ? process.env[fallback] : null);
}

function getStaffDbConfig() {
  const host = getEnv('STAFF_DB_HOST', 'DB_HOST');
  const user = getEnv('STAFF_DB_USER', 'DB_USER');
  const password = getEnv('STAFF_DB_PASSWORD', 'DB_PASSWORD');
  const database = getEnv('STAFF_DB_NAME', 'DB_NAME');
  const port = Number(getEnv('STAFF_DB_PORT', 'DB_PORT') || 3306);

  if (!host || !user || !password || !database) {
    throw new Error('[StaffShiftDB] Missing staff DB env vars: STAFF_DB_HOST, STAFF_DB_USER, STAFF_DB_PASSWORD, STAFF_DB_NAME');
  }
