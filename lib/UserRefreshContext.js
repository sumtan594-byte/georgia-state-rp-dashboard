...
function parseRateLimitHeaders(headers) {
  ...
  return {
    ...
    retryAfterMs: retryAfter !== null ? parseFloat(retryAfter) * 1000 : null,
    isGlobal: isGlobal === 'true',
    scope,
  };
}

function computeNextInterval(rateInfo, consecutiveErrors, currentInterval) {
  if (rateInfo.retryAfterMs !== null) {
    return Math.ceil(rateInfo.retryAfterMs) + 100;
  }

  if (rateInfo.remaining !== null && rateInfo.remaining <= 1 && rateInfo.resetAfterMs !== null) {
    return Math.ceil(rateInfo.resetAfterMs) + 100;
  }

  if (consecutiveErrors > 0) {
    return Math.min(currentInterval * ERROR_BACKOFF_MULTIPLIER, MAX_BACKOFF_MS);
  }
...
export function UserRefreshProvider({ children }) {
  ...
  const fetchRefreshedUser = useCallback(async () => {
    try {
      ...
      const rateInfo = parseRateLimitHeaders(res.headers);

      if (res.status === 429) {
        const backoffMs = rateInfo.retryAfterMs !== null
          ? Math.ceil(rateInfo.retryAfterMs) + 100
          : Math.max(rateInfo.resetAfterMs || 5000, 5000);
      ...
      if (!res.ok) {
        ...
        const backoff = computeNextInterval(rateInfo, consecutiveErrorsRef.current, currentIntervalRef.current);
      ...
      if (data.error) {
        const backoff = computeNextInterval(rateInfo, 0, currentIntervalRef.current);
        scheduleNextFetch(backoff);
        return;
      }
      ...
      const nextInterval = computeNextInterval(rateInfo, 0, currentIntervalRef.current);
      scheduleNextFetch(nextInterval);
    } catch {
      consecutiveErrorsRef.current++;
      const backoff = Math.min(currentIntervalRef.current * ERROR_BACKOFF_MULTIPLIER, MAX_BACKOFF_MS);
      scheduleNextFetch(backoff);
    }
  }, [scheduleNextFetch]);
