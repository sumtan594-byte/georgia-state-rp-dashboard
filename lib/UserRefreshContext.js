...
// Route changes and window focus already trigger an immediate refresh. A
// 5-second floor keeps roles current (fast auto-kick on role revocation) while
// avoiding excessive Discord API calls. At 5s interval, even 100 concurrent
// users produce ~20 req/s which is well within Discord rate limits.
...
function computeNextInterval(consecutiveErrors, currentInterval, rateLimit) {
  ...
  if (Number.isFinite(rateLimit.retryAfter) && rateLimit.retryAfter >= 0) {
    return Math.ceil(rateLimit.retryAfter * 1000) + 100;
  }

  const { remaining, resetAfter } = rateLimit;

  if (Number.isFinite(remaining) && Number.isFinite(resetAfter)) {
    if (remaining <= 0) {
...
export function UserRefreshProvider({ children }) {
  ...
  const scheduleNextFetch = useCallback((delayMs) => {
    ...
    // Invalid rate-limit metadata must never collapse the refresh loop into a
    // zero-delay request storm (setTimeout(NaN) is treated as immediate).
    const safeDelayMs = Number.isFinite(delayMs) && delayMs >= 1000
      ? delayMs
      : MIN_INTERVAL_MS;
    currentIntervalRef.current = safeDelayMs;
    timeoutRef.current = setTimeout(() => { if (fetchRef.current) fetchRef.current(); }, safeDelayMs);
  }, []);
  ...
  const fetchRefreshedUser = useCallback(async () => {
    ...
    try {
      ...
      if (res.status === 429) {
        ...
        const parsedRetryAfter = retryAfter === null ? NaN : Number.parseFloat(retryAfter);
        const backoffMs = Number.isFinite(parsedRetryAfter)
          ? Math.ceil(parsedRetryAfter * 1000) + 100
          : MIN_INTERVAL_MS;
  ...
  // When the tab regains visibility (user switches back), trigger an immediate
  // refresh so role revocations are detected within milliseconds rather than
  // waiting for the next scheduled polling cycle.
  useEffect(() => {
