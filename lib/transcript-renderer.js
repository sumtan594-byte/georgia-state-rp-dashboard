...
const WORKER_PATH = (() => {
  const local = path.join(__dirname, 'transcript-worker.js');
  if (fs.existsSync(local)) return local;
  return path.resolve(process.cwd(), 'lib/transcript-worker.js');
})();
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on('close', (code) => {
      if (code !== 0) {
        ...
        if (stderr) console.warn("[Worker Warning/Debug]:", stderr);
