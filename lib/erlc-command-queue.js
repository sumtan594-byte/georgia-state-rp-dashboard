...
const MAX_RETRIES = 4;
// Each queued command takes ~5.2s to drain, so 25 entries is already >2min of
// backlog; reject beyond that rather than accumulating promises unboundedly.
...
export function enqueueErlcCommand(command, erlcKey) {
  ...
  return new Promise((resolve, reject) => {
    if (q.queue.length >= MAX_QUEUE_SIZE) {
      reject(Object.assign(new Error('ER:LC command queue is full, try again shortly'), { status: 429 }));
      return;
    }
