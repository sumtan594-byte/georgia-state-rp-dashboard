...
export async function unbanMember(guildId, userId, reason) {
  const result = await discordFetch(
    ...
    {
      method: 'DELETE',
      headers: reason ? { 'X-Audit-Log-Reason': String(reason).slice(0, 512) } : {},
    },
    `Failed to unban ${userId} from ${guildId}`
  );

  // Discord returns 204 on success and 404 if the user was never banned
  // (or was already unbanned) - treat both as "not banned anymore".
