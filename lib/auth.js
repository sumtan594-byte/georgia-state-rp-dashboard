...
// Roles permitted to run 1:1 trainings (mirror the bot's "Train" button roles)
// plus the existing trainer role. Admins always pass.
...
export function canAccessTrainerHandbook(session) {
  return hasAnyRole(session, TRAINER_HANDBOOK_ROLES) || isAdmin(session);
}
