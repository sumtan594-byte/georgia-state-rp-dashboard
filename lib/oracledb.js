...
const DEFAULT_ORACLE_HOST = "adb.ap-sydney-1.oraclecloud.com";
const DEFAULT_ORACLE_PORT = 1522;
...
function getOracleConfig() {
  ...
  const config = {
    ...
    queueTimeout: parseInt(process.env.ORACLE_DB_QUEUE_TIMEOUT_MS || "120000", 10),
...
function getOracleEndpoint(connectString) {
  const hostMatch = String(connectString || "").match(/\(host\s*=\s*([^)]+)\)/i);
  const portMatch = String(connectString || "").match(/\(port\s*=\s*(\d+)\)/i);

  return {
    host: process.env.ORACLE_DB_HOST || hostMatch?.[1] || DEFAULT_ORACLE_HOST,
    port: parseInt(process.env.ORACLE_DB_PORT || portMatch?.[1] || String(DEFAULT_ORACLE_PORT), 10),
  };
}

async function checkOracleTcp(config, logPrefix) {
  const net = await import("net");
  const { host, port } = getOracleEndpoint(config.connectString);
  const timeoutMs = parseInt(process.env.ORACLE_DB_TCP_CHECK_TIMEOUT_MS || "5000", 10);
  ...
  return new Promise((resolve, reject) => {
    ...
    const done = (error) => {
      ...
      console.log(`${logPrefix} Startup check: TCP reachable ${host}:${port} in ${Date.now() - startedAt}ms`);
      resolve();
    };

    socket.setTimeout(timeoutMs, () => done(new Error(`TCP timeout connecting to ${host}:${port} after ${timeoutMs}ms`)));
...
async function checkOracleConnection({ logPrefix = "[OracleDB]" } = {}) {
  ...
  try {
    ...
    console.log(`${logPrefix} Startup check: pool min=${config.poolMin}, max=${config.poolMax}, queueTimeout=${config.queueTimeout}ms, connectTimeout=${config.connectTimeout}s`);
    ...
    await checkOracleTcp(config, logPrefix);
