...
async function checkOracleConnection({ logPrefix = "[OracleDB]" } = {}) {
  ...
  try {
    const config = getOracleConfig();
    console.log(`${logPrefix} Startup check: user=${config.user}, connectString=${config.connectString}, wallet=${config.configDir}`);

    const walletDir = config.configDir.replace(/\/+$/, "");
