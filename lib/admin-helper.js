...
export async function canReviewApplications(session) {
  const user = session?.user || session;
  if (!user?.id) return false;

  try {
    const [config, adminIds] = await Promise.all([
      getAuthConfig(),
      getAllAdminIds(),
    ]);
    const isAdmin = adminIds.includes(String(user.id));
    if (isAdmin) return true;

    const resource = getResource(config, 'applicationsReview');
    return userCanAccessResource({
      ...user,
      isAdmin,
    }, resource);
  } catch (e) {
    console.error('[AdminHelper] Applications config error:', e.message);
    return false;
  }
}
